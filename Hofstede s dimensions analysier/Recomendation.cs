...
{
    public class Recomendation      //class containing information about recomendations set
    {
        //===========hard-coded constants, beware changing!============
        public const int numberOfDimensions = 6;
        public const int numberOfLevels = 3;
        public const int numberOfUIfeatures = 10;
        //=============================================================

        private static string pathToHypercube = HttpContext.Current.Server.MapPath(@"~/docs/hypercube.txt");
        ...
        public static List<Recomendation> SetToDefault()        //method preparing default recomendations
        {
        ...
        public static string[, , , , ,] transformToCube(List<Recomendation> list) //method preparing hypercube of recomendation sets (do not forget to change number of dimensions!)
        {
            //Delete the hypercube-file if exists.
            if (File.Exists(HttpContext.Current.Server.MapPath(@"~/docs/hypercube.txt")))
            {
                File.Delete(HttpContext.Current.Server.MapPath(@"~/docs/hypercube.txt"));
            }

            // Create the file.
            using (FileStream fs = File.Create(HttpContext.Current.Server.MapPath(@"~/docs/hypercube.txt"))) { }

            string[, , , , ,] hypercube = new string[numberOfLevels, numberOfLevels, numberOfLevels, numberOfLevels, numberOfLevels, numberOfLevels];

            double numberOfRecomendations = Tools.generateNumberOfSets(numberOfLevels, numberOfDimensions); //getting number of recomendation sets.
            for (int i = 0; i < numberOfRecomendations; i++)
            {
                ...
                //writing hypercube to file
                System.IO.StreamWriter writer = new System.IO.StreamWriter(HttpContext.Current.Server.MapPath(@"~/docs/hypercube.txt"), true, Encoding.Default);
